	...
	class Admin extends CI_Controller {
		...
		public function fetch_online_leads(){
			...
			{
				echo "Failed to retraive data from 99acres";
			}
			echo "Success";
		}
		...
		$where="";
		        ...
		        $data['result'] = $this->callback_model->search_callback(null,$where,$offset,VIEW_PER_PAGE);
		 		...
		 		public function magicbrick()
		 		{
		 			//echo "magic_bricks_online_leads";
		 			$start_date = date("Y-m-d", strtotime('yesterday'));
					...
					$lead=$data->status;
					$count=$data->count;
					//echo "status".$lead."count".$count;
					
					
					if($count>0)
					{	
						for ($x = $count-1; $x >= 0; $x--) {
							echo $data->leads->lead[$x]->loginid."</br>";
   						 echo $data->leads->lead[$x]->name."</br>";
   						 echo $data->leads->lead[$x]->mobile."</br>";
   						 echo $data->leads->lead[$x]->email."</br>";
   						 echo $data->leads->lead[$x]->project."</br>";
   						 echo $data->leads->lead[$x]->id."</br>";
   						 echo $data->leads->lead[$x]->msg."</br>";
   						 echo $data->leads->lead[$x]->dt."</br></br>";
   						// echo $data->leads->lead->project;
						} 
